exports.handler = async (event) => {
  ...
  try {
    ...
    if (action === 'search') {
      ...
      const limit = Number.isFinite(requestedLimit) ? Math.min(Math.max(requestedLimit, 1), 75) : 10;
      ...
      const characters   = (idsData.characters   || []).slice(0, limit);
      const corporations = (idsData.corporations  || []).slice(0, limit);
      const alliances    = (idsData.alliances     || []).slice(0, limit);
